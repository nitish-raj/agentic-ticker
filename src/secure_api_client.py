...
from .sanitization import sanitize_error_message, sanitize_url
...
class SecureAPIClient:
    ...
    def _prepare_headers(self, endpoint: APIEndpoint, additional_headers: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    def _sanitize_url_params(self, url: str, params: Optional[Dict[str, Any]] = None) -> Tuple[str, Optional[Dict[str, Any]]]:
    ...
    def _make_request(
        ...
        if not rate_limiter.is_allowed():
            ...
            raise RateLimitError(f"Rate limit exceeded for {endpoint_name}. Wait {wait_time:.1f} seconds.")
        ...
        for attempt in range(request_max_retries + 1):
            try:
                ...
                if response.status_code == 401:
                    raise APIAuthenticationError(f"Authentication failed for {endpoint_name}")
                elif response.status_code == 403:
                    raise APIAuthenticationError(f"Access forbidden for {endpoint_name}")
                
                # Handle rate limiting from server
                if response.status_code == 429:
                    retry_after = response.headers.get('Retry-After', endpoint.retry_delay)
                    ...
                    if attempt < request_max_retries:
                        ...
                        raise RateLimitError(f"Rate limit exceeded for {endpoint_name} after {request_max_retries} retries")
                ...
                last_exception = e
                if attempt < request_max_retries:
                    logger.warning(f"Timeout for {endpoint_name}, retrying ({attempt + 1}/{request_max_retries})")
                    time.sleep(endpoint.retry_delay * (2 ** attempt))  # Exponential backoff
                    continue
                else:
                    logger.error(f"Timeout for {endpoint_name} after {request_max_retries} retries")
                    raise
            
            except requests.exceptions.ConnectionError as e:
                last_exception = e
                if attempt < request_max_retries:
                    logger.warning(f"Connection error for {endpoint_name}, retrying ({attempt + 1}/{request_max_retries})")
                    time.sleep(endpoint.retry_delay * (2 ** attempt))
                    continue
                else:
                    logger.error(f"Connection error for {endpoint_name} after {request_max_retries} retries")
                    raise
            
            except requests.exceptions.RequestException as e:
                last_exception = e
                if attempt < request_max_retries:
                    logger.warning(f"Request error for {endpoint_name}, retrying ({attempt + 1}/{request_max_retries})")
                    time.sleep(endpoint.retry_delay)
                    continue
                else:
                    logger.error(f"Request error for {endpoint_name} after {request_max_retries} retries")
                    raise
        ...
        if last_exception:
            ...
            raise requests.exceptions.RequestException(f"Request failed for {endpoint_name}")
...
def secure_gemini_request(path: str, method: str = 'POST', **kwargs) -> requests.Response:
...
def secure_coingecko_request(path: str, method: str = 'GET', **kwargs) -> requests.Response:
