    ...
    def compare_and_swap(self, key: str, expected_value: Any, new_value: Any) -> bool:
        ...
        with self.atomic_operation([key]):
            ...
            if current_value == expected_value:
                ...
                logger.debug(
                    f"Compare-and-swap failed for key: {key} (expected: {expected_value}, got: {current_value})"
                )
                return False

    def execute_with_lock(
        self, func: Callable, keys: Optional[list] = None, *args, **kwargs
    ):
        """
