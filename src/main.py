...
from .duplication_detector import CodeLocation

# Add src directory to path for imports
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

# Import models from src/models to avoid duplication
try:
    from models.code_location import CodeLocation
    from models.code_duplication_pattern import CodeDuplicationPattern
    from models.decorator import Decorator, DecoratorConcern
    from models.refactoring_progress import (
        RefactoringProgress,
        RefactoringPhase,
        RefactoringStatus,
    )
    from models.utility_module import UtilityModule
except ImportError:
...
# Pydantic models - imported from src/models/ to avoid duplication
...
class CodeDuplicationAnalysisRequest(BaseModel):
    analysis_scope: str = Field(
        ..., description="Directory path to scan (e.g., 'src/')"
    )
    file_patterns: List[str] = Field(
        ..., description="List of file patterns to include (e.g., ['*.py'])"
    )
    min_duplication_lines: int = Field(
        default=5, ge=0, description="Minimum lines for duplication detection"
    )
    ignore_comments: bool = Field(
        default=True, description="Whether to ignore comments in comparison"
    )
    ignore_whitespace: bool = Field(
        default=True, description="Whether to ignore whitespace in comparison"
    )
    include_tests: bool = Field(
        default=False, description="Whether to include test files"
    )
    threshold_similarity: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Threshold for similarity detection"
    )
...
@app.post(
    ...
    response_model=CodeDuplicationAnalysisResponse,
...
class CreateProgressRequest(BaseModel):
    ...
    refactoring_type: str = Field(
        ...,
        pattern=r"^(utility_module_creation|decorator_implementation|duplication_elimination)$",
    )
...
class UpdateProgressRequest(BaseModel):
    ...
    phase: Optional[str] = Field(
        None, pattern=r"^(analysis|planning|implementation|verification|completion)$"
    )
    status: Optional[str] = Field(
        None, pattern=r"^(not_started|in_progress|completed|failed|skipped)$"
    )
...
async def get_refactoring_progress(
    ...
    start_date: Optional[datetime] = Query(
        None, description="Start date for time range filter (ISO format)"
    ),
    end_date: Optional[datetime] = Query(
        None, description="End date for time range filter (ISO format)"
    ),
        ...
        if hasattr(progress, "id"):
            return {
                ...
                "started_at": (
                    ...
                    if hasattr(progress.started_at, "isoformat")
                    else str(progress.started_at)
                ),
        ...
        if hasattr(progress, "id"):
            return {
                ...
                "updated_at": (
                    ...
                    if hasattr(progress.updated_at, "isoformat")
                    else str(progress.updated_at)
                ),
            ...
            return {
                ...
                "progress_percentage": (
                    (request.tasks_completed / 10) * 100
                    if request.tasks_completed
                    else 0
                ),
                "tasks_completed": request.tasks_completed,
                "total_tasks": 10,
                ...
                "message": "Updated progress for test_module (test mode)",
            }
    ...
    try:
        ...
        if hasattr(progress, "id"):
            ...
            return {
                ...
                "status": "completed",
                "progress_percentage": 100,
                "message": "Refactoring marked as completed (test mode)",
            }
