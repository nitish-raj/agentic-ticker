...
def clean_numeric_data(
    ...
    cleaned_series = series.apply(
        lambda x: default_value if pd.isna(x) or x is None else x
    )
        ...
        for col, expected_type in column_types.items():
            if col in df.columns:
                if expected_type == "numeric" and not pd.api.types.is_numeric_dtype(
                    ...
                    logger.error(
                        f"Column '{col}' should be datetime but is {df[col].dtype}"
                    )
                    return False
                elif expected_type == "string" and not pd.api.types.is_string_dtype(
                    df[col]
                ):
                    logger.error(
                        f"Column '{col}' should be string but is {df[col].dtype}"
                    )
                    return False
...
def sanitize_forecast_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "trend" in sanitized_df.columns:
        ...
        sanitized_df["trend"] = sanitized_df["trend"].apply(
            lambda x: (
                "NEUTRAL"
                if pd.isna(x) or x is None or x not in valid_trends
                else str(x).upper()
            )
        )
...
def validate_numeric_range(
    ...
    if min_value is not None:
        validated_series = validated_series.apply(
            lambda x: max(min_value, x) if pd.notna(x) else x
        )

    if max_value is not None:
        validated_series = validated_series.apply(
            lambda x: min(max_value, x) if pd.notna(x) else x
        )
...
def validate_data_types(
    data: Any, expected_type: Union[type, tuple], allow_none: bool = False
) -> bool:
    ...
    if not isinstance(data, expected_type):
        logger.error(
            f"Data type {type(data)} does not match expected type {expected_type}"
        )
        return False
