...
def _format_json_for_display(obj) -> str:
    ...
    try:
        ...
        formatted = json.dumps(_json_safe(truncated_obj), indent=2,
                               ensure_ascii=False)
        return formatted
    except Exception:
        return str(obj)
...
def _truncate_large_data(obj, max_array_items=3, max_string_length=50):
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            truncated[key] = _truncate_large_data(value, max_array_items,
                                                  max_string_length)
        return truncated
    elif isinstance(obj, list):
        if len(obj) <= max_array_items:
            return [_truncate_large_data(item, max_array_items,
                                         max_string_length) for item in obj]
        else:
            # Show first few items and indicate truncation
            truncated = [_truncate_large_data(item, max_array_items,
                                              max_string_length)
