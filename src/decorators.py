        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if func.__name__.startswith("create_") and "chart" in func.__name__:
                    ...
                    return None
                elif func.__name__.startswith("process_") or func.__name__.startswith(
                    "transform_"
                ):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(
                    f"Completed {func.__name__} in {execution_time:.2f}s{result_str}"
                )
                return result
            except Exception as e:
                end_time = time.time()
                ...
                if param_name in bound_args.arguments:
                    ...
                    if validator == "dataframe":
                        ...
                        if not isinstance(value, list) or len(value) == 0:
                            raise ValueError(
                                f"Parameter '{param_name}' must be a non-empty list"
                            )
                    elif validator == "positive_number":
                        if not isinstance(value, (int, float)) or value <= 0:
                            raise ValueError(
                                f"Parameter '{param_name}' must be a positive number"
                            )
