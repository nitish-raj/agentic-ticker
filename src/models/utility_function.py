...
from src.exceptions import (
    ValidationError, ParameterError, UtilityFunctionError
)
...
class UtilityFunction(BaseModel):
    ...
    lines_of_code: int = Field(
        ...,
        description="Number of lines of code in the function",
        gt=0
    )
    complexity_score: Optional[float] = Field(
        None,
        description="Optional complexity score for code analysis",
        ge=0.0
    )
    is_decorated: bool = Field(
        False,
        description="Whether the function has decorators applied"
    )
