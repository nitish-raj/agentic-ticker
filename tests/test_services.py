...
class TestValidateTicker:
    ...
    def test_validate_crypto_ticker(
        self, mock_config, mock_secure_request, mock_classify, mock_ticker
    ):
...
class TestCompanyInfo:
    ...
    def test_get_company_info_success(self, mock_ticker):
        ...
        mock_ticker_instance = Mock()
        mock_ticker_instance.info = {"longName": "Apple Inc.", "shortName": "Apple"}
        mock_ticker.return_value = mock_ticker_instance
...
class TestComputeIndicators:
    ...
    def test_compute_indicators_with_small_dataset(self):
        ...
        for i, row in enumerate(result):
            ...
            assert isinstance(
                row["ma5"], (int, float)
            ), f"ma5 should be numeric in row {i}"
            assert isinstance(
                row["ma10"], (int, float)
            ), f"ma10 should be numeric in row {i}"
            assert isinstance(
                row["daily_return"], (int, float)
            ), f"daily_return should be numeric in row {i}"
            assert isinstance(
                row["volatility"], (int, float)
            ), f"volatility should be numeric in row {i}"
        ...
        assert (
            result[-1]["daily_return"] >= 0
        ), "Last day daily_return should be non-negative (upward trend)"
        assert (
            result[-1]["volatility"] >= 0
        ), "Last day volatility should be non-negative"
...
class TestBuildReport:
    ...
    def test_build_report_with_missing_data(self):
        ...
        result = build_report(
            ticker="TEST", events=[], forecasts=[], company_info=None, crypto_info=None
        )
