...
try:
    ...
    if config_security_path.exists():
        spec = importlib.util.spec_from_file_location(
            "config_security", config_security_path
        )
        if spec is None:
            raise ImportError(
                f"Could not load config security module from {config_security_path}"
            )
    ...
    if config_security_module and hasattr(
        config_security_module, "validate_security_config"
    ):
...
# Note: These imports are intentionally placed here after environment setup
    ...
    class MockOrchestrator:
        def run(self, ticker_input, days, threshold, forecast_days, on_event=None):
            ...
            steps = [
                ...
                (
                    "Event Detection",
                    f"Detecting significant price events (threshold: {threshold}%)",
                ),
                    ...
                    on_event(
                        {
                            "type": (
                                "info"
                                if i == 0
                                else "call" if i < len(steps) - 1 else "final"
                            ),
                            "name": step_name,
                            "message": description,
                            "step": i + 1,
...
def create_mock_data():
        ...
        indicator_data.append(
            {
                ...
                "bollinger_position": (
                    "Middle" if i % 3 == 0 else "Upper" if i % 3 == 1 else "Lower"
                ),
            }
        )
    ...
    forecast_dates = pd.date_range(
        start=end_date + timedelta(days=1), periods=5, freq="D"
    )
        ...
        forecasts.append(
            {
                ...
                "trend": (
                    ...
                    else "Bearish" if price < current_price else "Neutral"
                ),
            }
        )
...
def create_mock_report(ticker, days, threshold, forecast_days):
    ...
    return {
        ...
        "events": [
            {
                "date": "2024-01-15",
                "description": "Strong earnings report exceeded expectations",
                "magnitude": 3.2,
                "impact": "positive",
            },
            {
                "date": "2024-01-20",
                "description": "Market volatility due to economic news",
                "magnitude": -1.8,
                "impact": "negative",
            },
            {
                "date": "2024-01-25",
                "description": "Sector rotation into technology stocks",
                "magnitude": 2.1,
                "impact": "positive",
            },
        ],
...
def main():
    ...
    # Header section
    st.markdown(
        '<div class="main-header">🤖 Agentic Ticker</div>', unsafe_allow_html=True
    )
    st.markdown(
        '<div class="sub-header">Intelligent Stock & Cryptocurrency Analysis powered by Agentic AI</div>',
        unsafe_allow_html=True,
    )
    ...
    if is_mock_mode:
        st.markdown(
            '<div class="status-badge status-demo">📊 Demo Mode - Educational Purpose Only</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<div class="status-badge" style="background-color: #e8f5e8; color: #2e7d32; border: 1px solid #c8e6c9;">🚀 Live Mode - Real API Data</div>',
            unsafe_allow_html=True,
        )
    ...
    with col_params_center:
        ...
        with col1:
            ...
            threshold = st.slider(
                "Price Event Threshold (%)",
                0.5,
                10.0,
                2.0,
                0.1,
                help="Minimum price change to consider as significant event",
            )
        with col2:
    ...
    if get_session_state("analysis_running", False):
        ...
        try:
            if MOCK_MODE or Orchestrator is None:
                ...
                if api_key_available:
                    ...
                    print(
                        "🚀 Real orchestrator initialized successfully (API key available)"
                    )
                else:
        ...
        try:
            ...
            update_session_state(
                {"analysis_completed": True, "analysis_running": False}
            )
            st.rerun()
    ...
    if get_session_state("analysis_completed", False):
        ...
        tab_summary, tab_charts, tab_data, tab_report = st.tabs(
            ["📋 Summary", "📈 Charts", "📊 Data", "📄 Full Report"]
        )
            ...
            if report:
                ...
                with col1:
                    if "price_analysis" in report:
                        st.metric(
                            "Current Price", report["price_analysis"]["current_price"]
                        )
                        st.metric(
                            "30-Day Change",
                            report["price_analysis"]["price_change_30d"],
                        )
                with col2:
                    if "technical_indicators" in report:
                        st.metric("RSI", report["technical_indicators"]["rsi"])
                        st.metric(
                            "MACD Signal", report["technical_indicators"]["macd_signal"]
                        )
                with col3:
                    if "forecast" in report:
                        st.metric(
                            "5-Day Forecast", report["forecast"]["predicted_price_5d"]
                        )
                ...
                if "analysis_summary" in report:
                    ...
                    with col1:
                        st.write(
                            f"**Overall Rating:** {summary.get('overall_rating', 'N/A')}"
                        )
                        st.write(f"**Risk Level:** {summary.get('risk_level', 'N/A')}")
                        st.write(
                            f"**Investment Horizon:** {summary.get('investment_horizon', 'N/A')}"
                        )
                    with col2:
                        st.write(
                            f"**Trend:** {summary.get('key_strengths', ['N/A'])[0] if summary.get('key_strengths') else 'N/A'}"
                        )
                        st.write(
                            f"**Key Factors:** {', '.join(summary.get('key_factors', ['N/A'])[:2])}"
                        )
        ...
        with tab_charts:
            ...
            if price_data:
                try:
                    ...
                    st.info(
                        "Plotly not available for charts. Install with: pip install plotly"
                    )
            ...
            if report:
                ...
                if "price_analysis" in report:
                    ...
                    with col1:
                        ...
                        st.write(
                            f"**30-Day Change:** {price_data.get('price_change_30d', 'N/A')}"
                        )
                    with col2:
                        st.write(
                        ...
                        st.write(
                            f"**Trading Volume:** {price_data.get('trading_volume', 'N/A')}"
                        )
                ...
                if "technical_indicators" in report:
                    ...
                    with col2:
                        st.metric(
                            "Bollinger Position",
                            indicators.get("bollinger_position", "N/A"),
                        )
                        st.metric(
                            "50-Day MA", indicators.get("moving_average_50d", "N/A")
                        )
                    with col3:
                        st.metric(
                            "200-Day MA", indicators.get("moving_average_200d", "N/A")
                        )
                        st.metric(
                ...
                if "events" in report and report["events"]:
                    ...
                    for event in report["events"]:
                        impact_emoji = (
                            ...
                            else "📉" if event.get("impact") == "negative" else "➡️"
                        )
                        st.write(
                            f"{impact_emoji} **{event.get('date', 'N/A')}**: {event.get('description', 'N/A')} (Magnitude: {event.get('magnitude', 'N/A')}%)"
                        )
                ...
                if "forecast" in report:
                    ...
                    with col1:
                        st.write(
                            f"**Predicted Price (5d):** {forecast.get('predicted_price_5d', 'N/A')}"
                        )
                        st.write(f"**Confidence:** {forecast.get('confidence', 'N/A')}")
                    with col2:
                        st.write(f"**Trend:** {forecast.get('trend', 'N/A')}")
                        st.write(
                            f"**Expected Range:** {forecast.get('expected_range', 'N/A')}"
                        )

                    if "key_factors" in forecast:
                        st.write(
                            f"**Key Factors:** {', '.join(forecast['key_factors'])}"
                        )
                ...
                if "web_search_results" in report and report["web_search_results"]:
                    ...
                    for result in report["web_search_results"]:
                        st.write(
                            f"📰 [{result.get('title', 'N/A')}]({result.get('href', '#')})"
                        )
                        st.write(f"   {result.get('content', 'N/A')[:200]}...")
                        st.caption(
                            f"Source: {result.get('source', 'Unknown')} | {result.get('date', 'N/A')}"
                        )
    ...
    with st.sidebar:
        ...
        if is_mock_mode:
            ...
            st.markdown(
                '<div class="status-badge" style="background-color: #e8f5e8; color: #2e7d32; border: 1px solid #c8e6c9;">Live Mode</div>',
                unsafe_allow_html=True,
            )
