...
from src.orchestrator import Orchestrator, update_session_state
...
from src.decorators import handle_errors, log_execution, time_execution, validate_inputs
from src.validation_utils import validate_dataframe, validate_list_of_dicts
...
try:
    ...
    st.warning(f"Configuration initialization failed: {e}")
...
def main():
        ...
        if st.session_state.get('analysis_running', False):
            ...
            with logs_placeholder.container():
                ...
                try:
                    ...
                    # Build context dictionary for session state update
                    context = {}
                    for s in steps:
                        if s['type'] == 'result':
                            context[s['name']] = s['result']
                    
                    # Use the centralized session state update function
                    update_session_state(context)
                    
                    # Handle additional session state variables
                    st.session_state.web_search_results = context.get('ddgs_search')
                    st.session_state.validated_ticker = context.get('validate_ticker') or params['ticker']
    ...
    with col_report:
        ...
        # Check if we have results to display with validation
        required_keys = ['price_data', 'indicator_data', 'events', 'forecasts', 'report']
        has_results = all(key in st.session_state for key in required_keys)
        
        # Validate data structures before display
        if has_results and st.session_state.report:
            # Validate price data structure
            price_data_valid = validate_list_of_dicts(
                st.session_state.price_data, 
                required_keys=None, 
                min_length=0
            ) if st.session_state.price_data else True
            
            # Validate indicator data structure  
            indicator_data_valid = validate_list_of_dicts(
                st.session_state.indicator_data,
                required_keys=None,
                min_length=0
            ) if st.session_state.indicator_data else True
            
            # Only proceed if data is valid
            if price_data_valid and indicator_data_valid:
                ...
                st.warning("Data validation failed. Please run the analysis again.")
        else:
