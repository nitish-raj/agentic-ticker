...
def process_function_arguments(
    func_args: Dict[str, Any], context: Dict[str, Any]
) -> Dict[str, Any]:
...
def update_session_state(context: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if context.get("ddgs_search"):
            updates["web_search_results"] = context["ddgs_search"]
...
def get_function_success_message(func_name: str, result: Any) -> str:
    ...
    if func_name == "ddgs_search" and result:
        ...
        return "✅ Technical indicators computed: RSI, MACD, and Bollinger Bands calculated successfully."
        ...
        return "✅ Report built: Comprehensive analysis report completed with all findings."
...
def build_context_summary(context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for key, value in context.items():
        if value and key not in [
            "ticker_input"
        ]:  # Don't include original input in transcript
                ...
                transcript.append(
                    {
                        "type": "context",
                        "key": key,
                        "value": f"dict with keys: {list(value.keys())[:3]}",
                    }
                )
            else:
                transcript.append(
                    {"type": "context", "key": key, "value": str(value)[:50]}
                )
    return transcript
...
class Orchestrator:
    ...
    def tools_spec(self) -> List[Dict[str, Any]]:
        ...
        for name, fn in self.tools.items():
            spec.append(
                {"name": name, "docstring": fn.__doc__ or "", "signature": str(fn)}
            )
        return spec
    ...
    def run(
        ...
        if on_event:
            on_event(
                {
                    "type": "info",
                    "message": f"🤖 Starting agentic analysis for '{ticker_input}'...",
                }
            )
            on_event(
                {
                    "type": "info",
                    "message": f"📝 Initial analysis: User wants to analyze '{ticker_input}'. Let me understand what this asset is and gather comprehensive information.",
                }
            )
            ...
            if on_event:
                ...
                if step_count == 1:
                    on_event(
                        {
                            "type": "info",
                            "message": f"🧠 Strategic thinking: I need to first classify what type of asset '{ticker_input}' is, then gather the appropriate data for analysis.",
                        }
                    )
                ...
                if plan.call and plan.call.name:
                    ...
                    if func_name in self.tools:
                        try:
                            ...
                            processed_args = process_function_arguments(
                                func_args, context
                            )
                            ...
                            if result and on_event:
                                success_message = get_function_success_message(
                                    func_name, result
                                )
                                on_event({"type": "info", "message": success_message})

                            steps.append(
                                {"type": "result", "name": func_name, "result": result}
                            )
                            if on_event:
                                on_event(
                                    {
                            ...
                            if func_name == "validate_ticker" and result:
                                ...
                                if (
                                    ...
                                    and result.upper() != ticker_input.upper()
                                    and on_event
                                ):
                                    on_event(
                                        {
                                            "type": "info",
                                            "message": f"Converted '{ticker_input}' to ticker symbol: {result}",
                                        }
                                    )
