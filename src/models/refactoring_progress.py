...
class RefactoringProgress(BaseModel):
    ...
    phase: RefactoringPhase = Field(..., description="Current phase of refactoring")
    status: RefactoringStatus = Field(
        ..., description="Current status of the refactoring"
    )
    progress_percentage: float = Field(
        ..., ge=0.0, le=100.0, description="Progress percentage (0.0 to 100.0)"
    )
    ...
    lines_refactored: int = Field(
        ..., ge=0, description="Number of lines of code refactored"
    )
    ...
    notes: List[str] = Field(
        default_factory=list, description="Progress notes and observations"
    )
        ...
        if all(
            ...
            if total_tasks > 0:
                ...
                if abs(progress_percentage - expected_pct) > 5.0:  # Allow 5% tolerance
                    msg = (
                        f"Progress ({progress_percentage}%) vs tasks "
    ...
    def update_progress(
        ...
        self.progress_percentage = (
            (tasks_completed / self.total_tasks) * 100 if self.total_tasks > 0 else 0.0
        )
