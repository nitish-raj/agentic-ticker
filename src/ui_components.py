...
from .chart_utils import (
    ...
    create_chart_layout,
    create_price_traces,
    create_forecast_traces,
    create_animation_frames,
)
from .decorators import handle_errors, log_execution, validate_inputs
from .validation_utils import sanitize_forecast_data
...
def create_price_chart(
    price_data: List[Dict[str, Any]], indicator_data: List[Dict[str, Any]]
) -> go.Figure:
    ...
    if not price_df.empty:
        frames = create_animation_frames(price_df, create_price_traces, ind_df)
    ...
    animation_config = create_animation_controls(
        duration=500, transition_duration=300, button_prefix="Frame: "
    )
...
def create_forecast_chart(forecasts: List[Dict[str, Any]]) -> go.Figure:
    ...
    overall_trend = (
        forecast_df["trend"].iloc[-1] if not forecast_df.empty else "NEUTRAL"
    )
    ...
    def create_forecast_frame_traces(current_df, current_ind_df=None):
        ...
        current_trend = (
            current_df["trend"].iloc[-1] if not current_df.empty else "NEUTRAL"
        )
        current_avg_confidence = (
            current_df["confidence"].mean() if not current_df.empty else 0.5
        )
    ...
    frames = create_animation_frames(forecast_df, create_forecast_frame_traces)
    ...
    animation_config = create_animation_controls(
        duration=800, transition_duration=400, button_prefix="Day: "
    )
