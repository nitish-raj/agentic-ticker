...
class Sanitizer:
    ...
    def sanitize_api_keys(cls, text: str) -> str:
        ...
        sanitized = re.sub(
            r'(?:api[_-]?key|apikey|api[_-]?secret)\s*[:=]\s*["\']?[a-zA-Z0-9_\-]{16,}["\']?',
            lambda m: (
                f"{m.group(0).split(':')[0].split('=')[0]}=[REDACTED]"
        ...
        sanitized = re.sub(
            r'["\'][a-zA-Z0-9+/]{32,}={0,2}["\']', "[REDACTED]", sanitized
        )
    ...
    def sanitize_url(cls, url: str) -> str:
        ...
        sanitized = re.sub(
            r"token=[^&\s]+", "token=[REDACTED]", sanitized, flags=re.IGNORECASE
        )
        sanitized = re.sub(
            r"secret=[^&\s]+", "secret=[REDACTED]", sanitized, flags=re.IGNORECASE
        )
